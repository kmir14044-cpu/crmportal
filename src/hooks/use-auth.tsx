...
interface AccountMembershipSummary {
  accountId: string;
  accountName: string;
  role: AccountRole;
  ownerUserId: string;
...
interface AuthContextValue {
  ...
  /** Accounts this login can switch between. */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [activeAccountRole, setActiveAccountRole] = useState<AccountRole | null>(null);
  const [memberships, setMemberships] = useState<AccountMembershipSummary[]>([]);
      ...
      if (data) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (ctx.account?.id && ctx.account?.name) {
              accountRow = {
                id: ctx.account.id,
                name: ctx.account.name,
                default_currency:
                  ctx.account.default_currency ?? DEFAULT_CURRENCY,
              };
            }
            setMemberships(Array.isArray(ctx.memberships) ? ctx.memberships : []);
            setActiveAccountRole(isAccountRole(ctx.role) ? ctx.role : accountRole);
          } else {
            setActiveAccountRole(accountRole);
            setMemberships([]);
          }
        } catch (err) {
          console.error("[AuthProvider] fetchAccountContext threw:", err);
          setActiveAccountRole(accountRole);
          setMemberships([]);
        }
  ...
  }, [activeAccountRole, account?.id, profile?.account_role, profile?.account_id]);
