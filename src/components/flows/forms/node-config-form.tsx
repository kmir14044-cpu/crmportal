  ...
  switch (node.node_type) {
    ...
    case "start_flow":
      return (
        <div>
          <label className="mb-1 block text-xs text-muted-foreground">
            Target flow name or id
          </label>
          <Input
            value={
              (cfg as { target_flow?: string; flow_id?: string; flow_name?: string })
                .target_flow ??
              (cfg as { flow_id?: string }).flow_id ??
              (cfg as { flow_name?: string }).flow_name ??
              ""
            }
            onChange={(e) => onUpdateConfig({ target_flow: e.target.value })}
            placeholder="e.g. Packages or Lead capture"
            className="bg-muted font-mono text-xs"
          />
        </div>
      );
