...
> = {
  ...
  start_flow: {
    ...
    blurb: 'Jumps into another active flow',
    category: 'flow',
  },
  handoff: {
...
const NODE_HUE: Record<NodeType, { l: number; c: number; h: number }> = {
  ...
  start_flow: { l: 0.62, c: 0.18, h: 285 }, // violet — cross-flow jump
...
export function summarizeNode(node: BuilderNode): string | null {
  ...
  switch (node.node_type) {
    ...
    case 'start_flow': {
      const target =
        (typeof cfg.target_flow === 'string' && cfg.target_flow) ||
        (typeof cfg.flow_name === 'string' && cfg.flow_name) ||
        (typeof cfg.flow_id === 'string' && cfg.flow_id) ||
        '';
      return target ? `Flow ${truncate(target, 40)}` : 'No target flow selected';
    }
    case 'handoff': {
