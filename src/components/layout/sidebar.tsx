...
export function Sidebar({ open = false, onClose }: SidebarProps) {
  ...
  const switchAccount = async (accountId: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Account switch failed");
      window.location.href = pathname || "/dashboard";
    } catch (err) {
      console.error("[Sidebar] switchAccount failed:", err);
    ...
    <>
      ...
      >
        ...
        <div className="shrink-0 border-t border-border p-3">
          {memberships.length > 1 && account?.name ? (
            <DropdownMenu>
              <DropdownMenuTrigger
                className="mb-2 flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs text-muted-foreground hover:bg-muted/60 focus:bg-muted/60 focus:outline-none data-popup-open:bg-muted/60"
              ...
              >
                {memberships.map((membership) => {
                  const isActive = membership.accountId === account.id;
                  return (
                    <DropdownMenuItem
                      key={membership.accountId}
                      onClick={() => switchAccount(membership.accountId)}
