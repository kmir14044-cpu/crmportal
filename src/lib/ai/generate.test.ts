...
function config(overrides: Partial<AiConfig> = {}): AiConfig {
  return {
    ...
    autoReplyMaxPerConversation: 200,
...
describe('generateReply — Gemini', () => {
  it('calls Gemini generateContent and returns its text', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({
        ok: true,
    ...
    expect((opts?.headers as Record<string, string>)['x-goog-api-key']).toBe('AIza-x')
  })
})
