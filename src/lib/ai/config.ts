...
import type { AiConfig, EmbeddingsProvider } from './types'
...
const CONFIG_COLUMNS =
  'provider, model, api_key, system_prompt, is_active, auto_reply_enabled, auto_reply_max_per_conversation, embeddings_api_key, embeddings_provider'
...
export async function loadAiConfig(
  ...
  return {
    ...
    embeddingsApiKey,
    embeddingsProvider: row.embeddings_provider ?? 'openai',
  }
}
...
export async function loadEmbeddingsKey(
  db: SupabaseClient,
  accountId: string,
): Promise<{ key: string | null; provider: EmbeddingsProvider; corrupt: boolean }> {
  const { data, error } = await db
    .from('ai_configs')
    .select('embeddings_api_key, embeddings_provider')
    .eq('account_id', accountId)
    .maybeSingle()
  if (error || !data?.embeddings_api_key) return { key: null, provider: data?.embeddings_provider ?? 'openai', corrupt: false }
  try {
    return { key: decrypt(data.embeddings_api_key), provider: data.embeddings_provider ?? 'openai', corrupt: false }
  } catch {
    console.error(
      `[ai config] embeddings key for account ${accountId} could not be decrypted — check ENCRYPTION_KEY.`,
    )
    return { key: null, provider: data.embeddings_provider ?? 'openai', corrupt: true }
  }
}
