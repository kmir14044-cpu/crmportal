...
export async function ingestDocument(
  ...
  if (config.embeddingsApiKey) {
    try {
      embeddings = await embedTexts(config.embeddingsApiKey, chunks, config.embeddingsProvider ?? 'openai')
    } catch (err) {
      embedError = err
    }
  }
...
export async function retrieveKnowledge(
  ...
  if (config.embeddingsApiKey) {
    try {
      const [queryEmbedding] = await embedTexts(config.embeddingsApiKey, [query], config.embeddingsProvider ?? 'openai')
      if (queryEmbedding) {
