...
describe("validateFlowForActivation — nodes", () => {
  ...
  it("rejects a reachable cycle that can never end or hand off", () => {
    const nodes = [
      { node_key: "s", node_type: "start", config: { next_node_key: "loop" } },
      {
        node_key: "loop",
        node_type: "send_message",
        config: { text: "Still here", next_node_key: "loop" },
      },
    ];
    const issues = validateFlowForActivation(
      { ...validFlow, entry_node_id: "s" },
    ...
    expect(
      issues.some(
        (i) =>
          i.severity === "error" &&
          i.node_key === "loop" &&
          i.message.includes("never reaches an End or Handoff"),
      ),
    ).toBe(true);
  });

  it("allows a cycle when at least one reachable path exits to handoff", () => {
    const nodes = [
      { node_key: "s", node_type: "start", config: { next_node_key: "menu" } },
      {
        node_key: "menu",
        node_type: "send_buttons",
        config: {
          text: "Continue?",
          buttons: [
            { reply_id: "again", title: "Again", next_node_key: "menu" },
            { reply_id: "agent", title: "Agent", next_node_key: "h" },
          ],
        },
      },
      { node_key: "h", node_type: "handoff", config: {} },
    ...
    expect(
      issues.some((i) => i.message.includes("never reaches an End or Handoff")),
    ).toBe(false);
  });

  it("treats start_flow as a valid exit from the current flow", () => {
    const nodes = [
      { node_key: "s", node_type: "start", config: { next_node_key: "jump" } },
      {
        node_key: "jump",
        node_type: "start_flow",
        config: { target_flow: "Packages" },
      },
    ];
    const issues = validateFlowForActivation(
      { ...validFlow, entry_node_id: "s" },
  ...
  it("flags start_flow without a target flow name or id", () => {
    const nodes = [
      { node_key: "s", node_type: "start", config: { next_node_key: "jump" } },
      { node_key: "jump", node_type: "start_flow", config: { target_flow: "" } },
    ];
    const issues = validateFlowForActivation(
      { ...validFlow, entry_node_id: "s" },
      nodes,
    ...
    expect(
      issues.some((i) => i.node_key === "jump" && i.field === "target_flow"),
    ).toBe(true);
  });

  it("accepts legacy start_flow flow_id configs", () => {
    const nodes = [
      { node_key: "s", node_type: "start", config: { next_node_key: "jump" } },
      {
        node_key: "jump",
        node_type: "start_flow",
        config: { flow_id: "00000000-0000-0000-0000-000000000001" },
      },
    ];
    const issues = validateFlowForActivation(
      { ...validFlow, entry_node_id: "s" },
    ...
    expect(issues.some((i) => i.field === "target_flow")).toBe(false);
  });
});
...
describe("nodesThatCanReachTerminal", () => {
  it("marks upstream nodes that can reach end or handoff", () => {
    const set = nodesThatCanReachTerminal(validNodes);
    expect(set).toEqual(new Set(["start", "menu", "ho"]));
  });

  it("leaves closed loops unmarked", () => {
    const set = nodesThatCanReachTerminal([
      { node_key: "a", node_type: "start", config: { next_node_key: "b" } },
      {
        node_key: "b",
        node_type: "send_message",
        config: { text: "Loop", next_node_key: "a" },
      },
    ]);
