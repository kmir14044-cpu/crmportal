...
export interface DynamicUmrahListNodeConfig {
  source: "makkah_hotels" | "madinah_hotels" | "transport_sectors" | "ziyarat_places";
  text: string;
  button_label: string;
  header_text?: string;
  footer_text?: string;
...
export interface StartFlowNodeConfig {
   ...
   * at runtime so users do not need to copy ids from Supabase.
   */
  target_flow?: string;
  /** Legacy target flow UUID. Kept so already-authored nodes keep working. */
  flow_id?: string;
  /** Legacy/alternate target flow name. */
  flow_name?: string;
}
...
export type FlowNodeConfig =
  ...
  | { node_type: "dynamic_umrah_list"; config: DynamicUmrahListNodeConfig }
...
export interface KeywordTriggerConfig {
  ...
  keywords: string[];
  match_type?: "exact" | "contains" | "whole_word";
  case_sensitive?: boolean;
  /** When this keyword flow may start for a contact with no active run. */
  start_when?: "anytime" | "new_only" | "new_or_inactive";
  /** Used when start_when='new_or_inactive'. Defaults to 24. */
