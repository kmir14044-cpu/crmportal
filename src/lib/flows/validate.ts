...
export function validateFlowForActivation(
  ...
  if (flow.entry_node_id && keys.has(flow.entry_node_id)) {
    ...
    const canReachTerminal = nodesThatCanReachTerminal(nodes);
    for (const n of nodes) {
      ...
      } else if (!isFlowExitNode(n) && !canReachTerminal.has(n.node_key)) {
        issues.push({
          severity: "error",
          scope: "node",
          node_key: n.node_key,
          message:
            `Node "${n.node_key}" is on a reachable path that never reaches an End or Handoff node.`,
        });
      }
    }
  }
...
function validateNode(
  ...
  switch (node.node_type) {
    ...
    case "start_flow": {
      ...
      const target =
        cfg.target_flow?.trim() || cfg.flow_id?.trim() || cfg.flow_name?.trim();
      if (!target) {
        issues.push({
          severity: "error",
          scope: "node",
          node_key: node.node_key,
          field: "target_flow",
          message: "Start-flow needs a target flow name or id.",
        });
      }
      break;
    }
...
function outgoingEdges(node: NodeInput): string[] {
  switch (node.node_type) {
    ...
    case "start_flow":
      // Cross-flow edges are validated at runtime because the target
      // flow's graph is stored in another row set and may be edited or
...
function isTerminalNode(node: NodeInput): boolean {
  return node.node_type === "handoff" || node.node_type === "end";
}

function isFlowExitNode(node: NodeInput): boolean {
  return isTerminalNode(node) || node.node_type === "start_flow";
}

export function nodesThatCanReachTerminal(nodes: NodeInput[]): Set<string> {
  const byKey = new Map<string, NodeInput>();
  for (const n of nodes) byKey.set(n.node_key, n);
  ...
  while (changed) {
    changed = false;
    for (const n of nodes) {
      if (canReach.has(n.node_key)) continue;
      if (isFlowExitNode(n)) {
        canReach.add(n.node_key);
        changed = true;
        continue;
      }
      const outgoing = outgoingEdges(n).filter((key) => byKey.has(key));
      if (outgoing.some((key) => canReach.has(key))) {
        canReach.add(n.node_key);
        changed = true;
      }
    }
