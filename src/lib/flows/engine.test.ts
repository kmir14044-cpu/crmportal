...
describe("matchesKeywordTrigger", () => {
  ...
  it("default match_type='contains' does case-insensitive substring for longer keywords", () => {
  ...
  it("bounds short contains keywords so hi does not match inside other words", () => {
    const cfg = { keywords: ["hi"] };
    expect(matchesKeywordTrigger("hi", cfg)).toBe(true);
    expect(matchesKeywordTrigger("Hi there", cfg)).toBe(true);
    expect(matchesKeywordTrigger("say hi!", cfg)).toBe(true);
    expect(matchesKeywordTrigger("this is urgent", cfg)).toBe(false);
    expect(matchesKeywordTrigger("shipping issue", cfg)).toBe(false);
    expect(matchesKeywordTrigger("high priority", cfg)).toBe(false);
  });

  it("match_type='whole_word' requires token boundaries", () => {
    const cfg = { keywords: ["package"], match_type: "whole_word" as const };
    expect(matchesKeywordTrigger("package", cfg)).toBe(true);
    expect(matchesKeywordTrigger("show package price", cfg)).toBe(true);
    expect(matchesKeywordTrigger("packages", cfg)).toBe(false);
  });
...
describe("canStartFlowForConversation", () => {
  it("allows keyword flows anytime by default", () => {
    expect(
      canStartFlowForConversation({
        config: { keywords: ["hi"] },
  ...
  it("supports new customer only", () => {
    expect(
      canStartFlowForConversation({
        config: { keywords: ["hi"], start_when: "new_only" },
        isFirstInbound: true,
      }),
    ).toBe(true);
    expect(
  ...
  it("supports new or inactive customer windows", () => {
    ...
    expect(
      canStartFlowForConversation({
        ...
        isFirstInbound: false,
        lastCustomerMessageAt: "2026-07-28T11:59:00Z",
        now,
      }),
    ).toBe(true);
    expect(
      canStartFlowForConversation({
        ...
        isFirstInbound: false,
        lastCustomerMessageAt: "2026-07-28T12:01:00Z",
        now,
      }),
    ).toBe(false);
  });
...
describe("node classification helpers", () => {
  it("isAutoAdvancing covers start + send_message + send_media + condition + set_tag + start_flow", () => {
    ...
    expect(isAutoAdvancing("start_flow")).toBe(true);
  ...
  it("isSuspending covers the input-requiring nodes", () => {
    ...
    expect(isSuspending("start_flow")).toBe(false);
  ...
  it("isTerminal covers handoff + end", () => {
    ...
    expect(isTerminal("start_flow")).toBe(false);
...
describe("startFlowTargetRef", () => {
  it("uses target_flow first", () => {
    expect(
      startFlowTargetRef({
        target_flow: "Packages",
        flow_id: "id-1",
        flow_name: "Legacy",
      }),
    ).toBe("Packages");
  ...
  it("falls back to legacy flow_id and flow_name", () => {
    expect(startFlowTargetRef({ flow_id: "id-1" })).toBe("id-1");
    expect(startFlowTargetRef({ flow_name: "Lead capture" })).toBe(
      "Lead capture",
    );
  });
});
