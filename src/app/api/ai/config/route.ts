...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'provider, model, system_prompt, is_active, auto_reply_enabled, auto_reply_max_per_conversation, api_key, embeddings_api_key, embeddings_provider',
      )
...
export async function POST(request: Request) {
  try {
    ...
    maxPer = Math.min(2000, Math.max(1, Math.floor(maxPer)))
    ...
    const embeddingsProvider = (body.embeddings_provider ?? 'openai') as EmbeddingsProvider
    if (embeddingsProvider !== 'openai' && embeddingsProvider !== 'gemini') {
    ...
    const { data: existing } = await supabase
      .from('ai_configs')
      .select('id, provider, model, api_key, embeddings_provider')
      .eq('account_id', accountId)
      .maybeSingle()
