...
async function processMessage(
  ...
  const { data: priorCustomerMessages, count: priorCustomerMsgCount } = await supabaseAdmin()
    .from('messages')
  ...
  const lastCustomerMessageAt =
    Array.isArray(priorCustomerMessages) && priorCustomerMessages.length > 0
      ? (priorCustomerMessages[0] as { created_at?: string | null }).created_at ?? null
      : null
